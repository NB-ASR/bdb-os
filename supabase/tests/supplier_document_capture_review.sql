...
select ok(
  exists (select 1 from public.features where key = 'purchasing' and route = '/documents/purchasing' and is_active),
  'Purchasing feature metadata exists without deciding Production plan assignment'
);
...
select ok(
  exists (
    select 1
    from pg_constraint
    where conrelid = 'public.supplier_documents'::regclass
      and contype = 'f'
      and pg_get_constraintdef(oid) ilike 'FOREIGN KEY (workspace_id, id) REFERENCES documents(workspace_id, id) ON DELETE RESTRICT%'
  ),
  'Purchasing metadata reuses the canonical workspace Document identity'
);

select ok(
  pg_get_functiondef('private.supplier_document_actor_can_write(uuid,uuid,text)'::regprocedure)
    not ilike '%platform_support_sessions%',
  'Purchasing writes do not depend on Integration support sessions'
);

select ok(
  has_table_privilege('service_role', 'public.supplier_documents', 'SELECT'),
  'service role can read supplier documents for trusted commands'
);
select ok(
  has_table_privilege('service_role', 'public.supplier_document_lines', 'SELECT'),
  'service role can read supplier document lines for trusted commands'
);
select ok(
  has_table_privilege('service_role', 'public.supplier_document_extraction_runs', 'SELECT'),
  'service role can read extraction runs for trusted commands'
);
select ok(
  has_table_privilege('service_role', 'public.supplier_document_command_receipts', 'SELECT'),
  'service role can read command receipts for idempotency'
);
