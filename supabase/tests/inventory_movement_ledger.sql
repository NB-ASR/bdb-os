...
select has_function(
  'private',
  'inventory_actor_can_view_feature',
  array['uuid','uuid','text'],
  'cross-feature source visibility guard exists'
);
...
select ok(has_table_privilege('service_role','public.inventory_locations','INSERT'), 'service role can execute trusted location commands after explicit-grant enforcement');
select ok(has_table_privilege('service_role','public.inventory_movements','INSERT'), 'service role can execute trusted movement commands after explicit-grant enforcement');
select ok(has_table_privilege('service_role','public.inventory_command_receipts','INSERT'), 'service role can persist Inventory idempotency receipts after explicit-grant enforcement');
