...
select ok(position('calendar_availability_command_receipts' in pg_get_functiondef('public.apply_calendar_availability_command_legacy(uuid,text,text,text,uuid,uuid,uuid,integer,uuid,smallint,time without time zone,time without time zone,timestamp without time zone,timestamp without time zone,text,text,text,text,boolean)'::regprocedure)) > 0, 'availability business rules store idempotency receipts');
select ok(position('activity_items' in pg_get_functiondef('public.apply_calendar_availability_command_legacy(uuid,text,text,text,uuid,uuid,uuid,integer,uuid,smallint,time without time zone,time without time zone,timestamp without time zone,timestamp without time zone,text,text,text,text,boolean)'::regprocedure)) > 0, 'availability business rules write Activity history');
