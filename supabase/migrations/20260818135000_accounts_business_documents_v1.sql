...
-- Catalogue-aware Invoice draft lines. Existing RPC signature remains unchanged.
...
begin
  ...
  if p_action='create' then
    ...
    if p_action='update' then
      ...
      -- Re-run source quantity checks against the stored draft lines before issuing.
      if source_type_value='invoice' then
        if exists (
...
-- Extend workspace configuration atomically while preserving the existing trusted settings command.
...
-- Extend Customer command with a VAT identifier without duplicating Customer lifecycle logic.
