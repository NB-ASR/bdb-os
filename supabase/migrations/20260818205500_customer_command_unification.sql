...
-- Customer creation must expose one authoritative PostgREST RPC signature.
-- The Accounts document release temporarily added a VAT-aware overload beside
-- the original function. PostgREST clients should not have to resolve two
-- overlapping functions for the same command path.
...
-- Remove the overlapping legacy overload only after the VAT-aware function no
-- longer depends on it. This leaves one stable RPC contract for the API.
drop function if exists public.apply_customer_command(uuid, uuid, text, text, uuid, uuid, integer, text, text, text, text, text, text, text, jsonb, boolean);
