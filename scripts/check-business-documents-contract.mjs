...
const partialCreditSalesOrderMigration = await readFile("supabase/migrations/20260819130000_accounts_partial_credit_sales_order.sql", "utf8");
...
assert.match(partialCreditSalesOrderMigration, /sales_order_reference/);
assert.match(partialCreditSalesOrderMigration, /A Sales Order \(SO\) number is required because this Invoice contains Products/);
assert.match(partialCreditSalesOrderMigration, /line\.line_type = 'product'/);
assert.match(partialCreditSalesOrderMigration, /snapshot_credit_note_sales_order/);
assert.match(partialCreditSalesOrderMigration, /write_credit_note_amount_lines/);
assert.match(partialCreditSalesOrderMigration, /allocation_factor := target_amount \/ remaining_source_total/);
assert.match(partialCreditSalesOrderMigration, /totals\.total <> target_amount/);
assert.match(finalDocumentsApi, /p_sales_order_reference/);
...
assert.match(accounts, /SO number/);
assert.match(accounts, /Required for Products/);
assert.match(accounts, /Invoice number/);
assert.match(accounts, /Amount to credit/);
assert.match(accounts, /Remaining after credit/);
assert.match(accounts, /Inherited from Invoice/);
assert.doesNotMatch(accounts, /Quantity to credit/, "Normal Credit Note creation must be amount-first rather than quantity-first.");
...
assert.match(renderer, /salesOrderReference/);
assert.match(renderer, /SO number/);
...
console.log("Business Documents contract preserves final-first creation, exact amount-based Credit Notes, Product SO references, document safety and restrained Accounts UX.");
