...
Engineering implementation is complete on the integration branch. General Documents uses trusted server commands, typed links, ordered offline queues and permission-aware Customer 360 read models. The remaining authenticated acceptance and production migration gates belong to the wider PR #23 release process rather than this department integration.
...
The existing free-text `documents.linked_to` field is legacy display data. It is not a reliable integration contract and is not used for new links.

## Business problem

The legacy Document Library could upload a file and attach a free-text label, but it could not reliably answer:
...
Free-text linkage created ambiguous records and made cross-department navigation fragile.
...
The existing `public.documents` table and stored objects remain authoritative. The integration extends that table rather than creating a replacement catalogue.
...
Legacy `customer_id` and `linked_to` values remain compatibility data only. Current Customer 360 counts, lists and activity use `document_links`.

## Command boundary

Browser clients cannot insert, update or delete Documents, Document links or command receipts directly. Browser storage policies also do not permit direct writes to the General Documents bucket.
...
The legacy direct-write upload route and browser write grants were removed in the same cutover as the trusted API and offline queue.

## Permission boundary

Documents permission remains authoritative for reading and changing Document records.

Opening or linking a Document through another department also requires access to that source context. The read API does not expose restricted source-record details merely because the user can view Documents.
...
Customer 360 reads Customer-linked Documents through active typed links. It does not copy Document metadata into a Customer-owned table.

The operational summary counts distinct Documents through current Customer links. Unified history includes Document upload, Customer-link creation, Customer-link revocation and Document archive events.

Document commands do not create, modify or settle Appointments, Sales, Invoices, Payments, Communications, Inventory movements or Banking records.

## Offline boundary

After one successful online load, the General Documents index is cached per workspace.

Version 1 offline-capable commands include:
...
File bytes are stored in IndexedDB. Metadata actions and file uploads retain stable identities, replay in order and stop on the first conflict. A file is not shown as confirmed until storage and database creation both succeed.
...
- RLS and privilege assertions
- pgTAP schema and security tests
- Rolled-back create, retry, link, revoke and archive lifecycle validation
- Zero-residue cleanup checks
- Source-department side-effect checks
- TypeScript and production-build validation through the integration preview
...
- Cached index and ordered offline commands
