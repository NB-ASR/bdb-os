...
- the existing canonical `documents` record for the original file;
- `supplier_documents`
...
The `supplier_documents` row reuses the canonical Document ID and has a workspace-safe foreign key to `documents`. Purchasing therefore adds review metadata without creating a second file identity.
...
The scanner does not silently create Suppliers, Products or Product–Supplier relationships. Unmatched values remain review work until the responsible catalogue workflow creates and approves the master record.
...
- Main-owned active-profile, workspace-membership, feature and member-permission checks remain authoritative.
