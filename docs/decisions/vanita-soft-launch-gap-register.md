...
- A successful encrypted Production database backup and restore-list
  verification must be recorded before real client data is entered.
- The duplicate Vercel project must not be disconnected until its environment
  and integration parity is visible; `bdb-os` is the canonical nominee because
  it owns the live Production aliases.
...
- The real Vanita owner identity is not yet available; acceptance uses
  rollback-only QA identities until it is supplied.
