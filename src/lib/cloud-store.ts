...
export async function loadCloudWorkspace(): Promise<{ state: BdbState; workspaceId: string; role: string } | null> {
  ...
  const preferred = document.cookie.match(/(?:^|; )bdb-workspace=([^;]+)/)?.[1];
  let membershipQuery = supabase.from("workspace_memberships").select("workspace_id,role,status,workspaces!inner(name,status)").eq("status", "active");
  if (preferred) membershipQuery = membershipQuery.eq("workspace_id", preferred);
  const { data: memberships } = await membershipQuery.limit(1);
  const membership = memberships?.[0] as { workspace_id: string; role: string; workspaces: { name: string } } | undefined;
  ...
  const settingsRow = row(settings.data ?? {}); const themeRow = row(themes.data ?? {});
  ...
  const theme: WorkspaceTheme = {
    preset: (themeRow.preset as WorkspaceTheme["preset"]) ?? "obsidian-gold", mode: (themeRow.mode as WorkspaceTheme["mode"]) ?? "dark", accentColor: String(themeRow.accent_color ?? "#d3a84b"), fontFamily: (themeRow.font_family as WorkspaceTheme["fontFamily"]) ?? "manrope", textScale: Number(themeRow.text_scale ?? 1), density: (themeRow.density as WorkspaceTheme["density"]) ?? "comfortable", highContrast: Boolean(themeRow.high_contrast), reducedMotion: Boolean(themeRow.reduced_motion), clientLogoPath: themeRow.client_logo_path ? String(themeRow.client_logo_path) : undefined, clientLogoUrl,
  };
  const state: BdbState = {
    settings: { businessName: membership.workspaces.name, ownerName: String(settingsRow.owner_name ?? "Owner"), email: String(settingsRow.email ?? ""), phone: String(settingsRow.phone ?? ""), currency: (settingsRow.currency as "GBP" | "EUR" | "USD") ?? "GBP", invoicePrefix: String(settingsRow.invoice_prefix ?? "INV"), vatRate: Number(settingsRow.vat_rate ?? 20) }, theme,
  ...
  return { state: { ...seedState, ...state }, workspaceId, role: membership.role };
}

export async function cloudInsert(table: string, values: Row | Row[]) { if (!isSupabaseConfigured()) return; const { error } = await createClient().from(table).insert(values); if (error) console.error(`[BDB sync] ${table}`, error.message); }
export async function cloudUpdate(table: string, values: Row, id: string) { if (!isSupabaseConfigured()) return; const { error } = await createClient().from(table).update(values).eq("id", id); if (error) console.error(`[BDB sync] ${table}`, error.message); }
export async function cloudUpsert(table: string, values: Row, onConflict: string) { if (!isSupabaseConfigured()) return; const { error } = await createClient().from(table).upsert(values, { onConflict }); if (error) console.error(`[BDB sync] ${table}`, error.message); }

export async function uploadWorkspaceLogo(workspaceId: string, file: File) {
  const supabase = createClient(); const safeName = file.name.toLowerCase().replace(/[^a-z0-9.-]/g, "-"); const path = `${workspaceId}/branding/${crypto.randomUUID()}-${safeName}`;
...
export async function uploadWorkspaceDocument(workspaceId: string, file: File) {
  const supabase = createClient(); const safeName = file.name.toLowerCase().replace(/[^a-z0-9.-]/g, "-"); const path = `${workspaceId}/documents/${new Date().getFullYear()}/${crypto.randomUUID()}-${safeName}`;
