...
const QUEUE_PREFIX = "bdb-inventory-queue-v2";
...
export function enqueueInventoryCommand(
  ...
  if (!queue.some((item) => item.id === id)) writeInventoryQueue(workspaceId, [...queue, command]);
  return command;
}
...
export async function submitInventoryCommand(command: InventoryQueuedCommand) {
  ...
  if (!response.ok || !result.ok) throw new Error(result.error ?? "Inventory command failed.");
...
export async function flushInventoryQueue(
  ...
  if (typeof navigator !== "undefined" && !navigator.onLine) {
    return { completed: 0, remaining: readInventoryQueue(workspaceId).length };
  }
  let completed = 0;
  for (const command of readInventoryQueue(workspaceId)) {
    try {
      ...
      failInventoryCommand(
        workspaceId,
        command.id,
        error instanceof Error ? error.message : "Inventory command failed.",
      );
      break;
    }
  }
