...
import { OperatorControlRoom } from "@/components/operator-control-room";
...
export function SoloOperatorExperience() {
  ...
  const { state, ready, mode, workspaceId, role, syncStatus, lastError, clearError } = useBdb();
  ...
  const renderOperator = () => (
    <>
      <PageHeader
        ...
        title="Boringly reliable work, with proof."
        description="Every workflow is tied to a live record, a published Sector Pack policy, an approval boundary and a final evidence trail. Simulation is kept visibly separate from verified work."
      />
      <OperatorControlRoom
        preview={preview}
        workspaceId={workspaceId}
        actions={snapshot.actions}
        currency={workingState.settings.currency}
        blueprint={workingState.blueprint}
      />
