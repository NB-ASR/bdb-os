...
function removeReviewCache(workspaceId: string, documentId: string) {
  if (typeof window === "undefined") return;
  window.localStorage.removeItem(reviewCacheKey(workspaceId, documentId));
}
...
export default function PurchasingWorkspace() {
  ...
  const [pendingReviews, setPendingReviews] = useState<PendingReviewCommand[]>([]);
  ...
  }, [loadCloud, loadLocalState, online, scanDocument, syncPendingReviews, workspaceId]);

  useEffect(() => {
    if (!cloudMode || !online || !workspaceId || workspaceId === "demo") return;
    const key = `${workspaceId}:online`;
    if (autoSyncKey.current === key) return;
    autoSyncKey.current = key;
    void syncPending();
  }, [cloudMode, online, syncPending, workspaceId]);

  useEffect(() => {
    if (!online) autoSyncKey.current = "";
  }, [online]);
  ...
  async function discardReviewDrafts() {
    if (!workspaceId || busy) return;
    const queued = readReviewQueue(workspaceId);
    for (const command of queued) removeReviewCache(workspaceId, command.documentId);
    writeReviewQueue(workspaceId, []);
    setPendingReviews([]);
    if (review && queued.some((command) => command.documentId === review.document.id)) setReview(null);
    if (online) await loadCloud().catch(() => undefined);
    setNotice("Local review drafts were discarded. Shared supplier documents were not changed.");
  }

  function resetReviewCommand() {
    activeReviewCommand.current = null;
  }
  ...
  async function saveReview(action: "save_review" | "approve") {
    ...
    if (!online) {
      ...
      setPendingReviews(readReviewQueue(workspaceId));
    ...
    setError("");
    setNotice("");
    const command = activeReviewCommand.current?.action === action
      ? activeReviewCommand.current
      : { action, key: crypto.randomUUID() };
    activeReviewCommand.current = command;
    try {
  ...
  return (
    <>
      ...
      {pendingUploads.length || pendingReviews.length ? (
        <div className="settings-note" style={{ marginBottom: 18 }}>
          <strong>{pendingUploads.length} file{pendingUploads.length === 1 ? "" : "s"} and {pendingReviews.length} review draft{pendingReviews.length === 1 ? "" : "s"} waiting to sync</strong>
          ...
          {pendingReviews.length ? (
            <div className={styles.pendingList}>
              {pendingReviews.map((item) => (
                <div className={styles.pendingItem} key={item.id}>
                  <div className={styles.pendingMeta}>
                    <strong>Review draft · {item.documentId.slice(0, 8)}</strong>
                    <span>{item.lines.length} line{item.lines.length === 1 ? "" : "s"} · {item.attempts ? `${item.attempts} failed attempt${item.attempts === 1 ? "" : "s"}` : "Waiting"}</span>
                    {item.lastError ? <small>{item.lastError}</small> : null}
                  </div>
                </div>
              ))}
          ...
          <div className={styles.pendingActions}>
            ...
            {pendingReviews.length ? <Button variant="quiet" onClick={() => void discardReviewDrafts()} disabled={busy}><Trash2 size={15} /> Discard review drafts</Button> : null}
          </div>
        </div>
      ) : null}
