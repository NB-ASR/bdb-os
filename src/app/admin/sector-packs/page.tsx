...
export default function SectorPacksPage() {
  ...
  const load = useCallback(async () => {
    ...
    setData(dashboard);
    setSelectedWorkspaceId((current) => current && dashboard.workspaces.some((workspace) => workspace.id === current)
      ? current
  ...
  useEffect(() => {
    ...
    const existing = data.configs.find((config) => config.workspace_id === selectedWorkspaceId);
    setSelectedPackId(existing?.sector_pack_id ?? data.packs[0]?.id ?? "");
      ...
      <div className={styles.shell}>
        <header className={styles.header}>
          ...
          <div className={styles.actions}>
            ...
            <button className="button button-secondary" onClick={() => void load()} disabled={busy !== ""}><RefreshCw size={16} /> Refresh</button>
            ...
            <section className={styles.panel}>
              ...
              <div className={styles.clientList}>
                ...
                {data.workspaces.map((workspace) => {
                  ...
                  return (
                    <button key={workspace.id} className={`${styles.clientButton} ${selectedWorkspaceId === workspace.id ? styles.clientButtonActive : ""}`} onClick={() => setSelectedWorkspaceId(workspace.id)}>
