...
export default function AccountsOverviewPage() {
  ...
  function discardPending(command: AccountsQueuedCommand) {
    if (!bundle?.workspaceId || !canDiscardAccountsCommand(command)) return;
    if (!window.confirm("The server rejected this Accounts change before it could be accepted. Discard the rejected local command? This cannot be undone.")) return;
  ...
  return (
    <main className={styles.workspace}>
      ...
      {pendingCommands.length ? <section className={styles.detailCard}><h3><RefreshCw size={16} /> Pending sync · {pendingCommands.length}</h3><p className={styles.muted}>Queued financial changes are preserved locally and applied in order. A failed command stops the queue for review. If the outcome is uncertain after a network or server interruption, BDB keeps the command for idempotent retry rather than letting it be discarded.</p><div className={styles.linkList}>{pendingCommands.slice(0, 5).map((command, index) => <div className={styles.linkRow} key={command.id}><span><strong>{actionLabel(command.action)}</strong><span className={styles.subtle}>{command.lastError || `Queued ${formatDate(command.createdAt)}`}</span></span>{index === 0 && canDiscardAccountsCommand(command) ? <button className={styles.secondaryLink} type="button" onClick={() => discardPending(command)}><Trash2 size={14} /> Discard rejected change</button> : index === 0 && command.lastError ? <span className={styles.muted}>Retry required</span> : <span className={styles.muted}>Waiting</span>}</div>)}</div><div className={styles.inlineActions}><button className={styles.secondaryLink} type="button" disabled={syncing || !navigator.onLine} onClick={() => void retryPending()}><RefreshCw size={14} /> {syncing ? "Synchronising…" : "Retry queue"}</button></div></section> : null}
