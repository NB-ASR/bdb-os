...
export async function PATCH(request: Request) {
  try {
    ...
    if (effectiveProfile === "custom" && membership.access_profile !== "custom") {
      // Prepare the custom permission set before changing the effective profile.
      // If the membership update fails, these rows remain inert under the old profile.
    ...
    if (effectiveProfile !== "custom") {
      // Standard profiles ignore per-member overrides. Clean stale rows only
      // after the profile switch so a failed update cannot strip live access.
