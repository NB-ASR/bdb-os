...
type Row = { id: string; number: string; invoice_id: string; customer_id: string; customer_name_snapshot: string; currency: string; reason: string; status: string; issued_at: string | null; total_amount: number; created_at: string; sales_order_reference: string | null };
...
function decodeCursor(value: string | null): Cursor | null {
  ...
  } catch { throw new CommandError("INVALID_ACCOUNTS_CURSOR", "Credit Note page cursor is invalid."); }
}
...
export async function GET(request: Request) {
  return runCommand(async () => {
    ...
    if (status !== "all" && !STATUSES.has(status)) throw new CommandError("INVALID_ACCOUNTS_INPUT", "Credit Note status filter is invalid.");

    let query = supabase.from("credit_notes")
      .select("id,number,invoice_id,customer_id,customer_name_snapshot,currency,reason,status,issued_at,total_amount,created_at,sales_order_reference")
      .eq("workspace_id", workspaceId)
      .order("created_at", { ascending: false }).order("id", { ascending: false }).limit(limit + 1);
    if (q) { const pattern = `%${q}%`; query = query.or(`number.ilike.${pattern},customer_name_snapshot.ilike.${pattern},reason.ilike.${pattern},sales_order_reference.ilike.${pattern}`); }
