...
export default function WorkspacePage() {
  ...
  const load = useCallback(async () => {
    ...
    setCurrentWorkspaceName(String(context.currentWorkspaceName ?? "").trim());
    setCurrentUserName(String(context.currentUser?.fullName ?? "").trim());
    const hasBranding = Boolean(context.branding?.enabled);
    setBrandingEnabled(hasBranding);
    setClientLogoUrl(hasBranding ? String(context.branding?.logoUrl ?? "") : "");
  ...
  const statusLine = bundle?.cached
    ...
    : bundle?.generatedAt
      ...
      : "";
  const companyName = bundle?.workspaceName || currentWorkspaceName || "Your business";
  const greetingName = firstName(currentUserName);
  const logoUrl = brandingEnabled && clientLogoUrl ? clientLogoUrl : "";
